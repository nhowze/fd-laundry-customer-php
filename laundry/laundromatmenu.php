   ...
   Stripe.setPublishableKey("<?php echo $keys['Key']; ?>");
            function stripeResponseHandler(status, response) {
                if (response.error) {
                    ...
                    var form$ = $("#payment-form").attr('action', 'addorder.php');
...
<style>
 ...
 color:black;
	...
	echo'<script src="moment.js"></script>
<script>
...
$result2 = mysqli_query($mysqli, $sql2);
							...
							if(mysqli_num_rows($result2) == 0){
							   ...
							   echo'<h3>Sorry but there are no options currently avaiable :(</h3>';
									...
									echo'<form id="payment-form"  name="orderForm" method="post" enctype="multipart/form-data"   action="addorder.php" onsubmit="return validateForm(this);">	
...
if($detect->isMobile()) {
						...
						<span class="image avatar"><img src="https://'.$_SERVER['SERVER_NAME'].'/Laundromats/'.$row2['Image'].'"  alt="" /></span>
						<br><br>
						</td>
						<tr>
...
<td style="width:10%;"><label class="container2" style="display:inline;">
								...
								<td style="width:50%;">$'.$optionrow['Price'].'</td>
...
<td style="width:10%;"><label class="container2" style="display:inline;">
					...
					//options pc end
						...
						echo'<div style="width:100% !important;">	Delivery/Pickup Address: <br>'.$row['Address'].' '.$row['Unit'].'  '.$row['City'].', '.$row['State'].'
							'.$zip.'<br><a class="button" href="../account.php#address">Change Address</a><br><br></div>';
							
						}else{
						    
						    
						    
						    	echo'<div style="width:100% !important;">	Delivery/Pickup Address: '.$row['Address'].' '.$row['Unit'].'  '.$row['City'].', '.$row['State'].'
							'.$zip.'<br> <a class="button" href="../account.php#address">Change Address</a> <br><br></div>';
...
if($detect->isMobile()) {
				...
				echo'<ul style="width:100%; list-style-type:none; margin-bottom:-20%;">';
							
					}else{

						echo'<ul style="width:100%; list-style-type:none; margin-bottom:-5%;">';
...
echo'</table>';
	...
	echo'</div>';
		echo'	<div style=" margin-left:-10%; margin-top:-15%;">';
}else{
...
echo'<div style="visibility:hidden;"><label class="container2">Est. Items Total: <strong id="totalprice" style="float:right;">$0.00<br></strong></label></div>
				<label class="container2">Est. Items Total: <strong  style="float:right;">TBD<br></strong></label>
				<label class="container2">Est. Delivery Fee: <strong id="Deliverytotal" style="float:right;">$0.00<br></strong></label>

<label class="container2" style="font-size:100%;">
Minimum $15 order</label>
				<div style="visibility:hidden;">

				<label class="container2">Est. Tax: <strong id="Taxtotal" style="float:right;">$0.00<br></strong></label>
				<label class="container2">Est. Total: <strong id="Totalorder" style="float:right;">$0.00<br></strong></label></div></div>
