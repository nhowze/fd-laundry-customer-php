...
$sql = "SELECT * FROM users WHERE username = '".$_POST['username']."' AND ID <> ".$row['id']." ";
$result = mysqli_query($mysqli, $sql);
...
$sql = "SELECT * FROM users WHERE email = '".$_POST['email']."' AND ID <> ".$row['id']." ";
...
if($row['Phone'] != $_POST['phone']){
	...
	if($answer!= "error"){
		...
		$mysqli->query("UPDATE users SET Phone= '".$_POST['phone']."', Phone_Confirmed  = 'False', SMS_Address = '".$answer."', Phone_Code = '".$code."', username  = '".$_POST['username']."' WHERE username = '".$_SESSION['username']."' ");
...
$_SESSION['accountf'] ="Verfication Code Sent";
...
try {
	...
	if($_POST['email'] != ""){
		\Stripe\Customer::update($row['Stripe'], [
			'email' => $_POST['email'],
	]);
	}
...
$target_dir = $_SERVER["DOCUMENT_ROOT"]."/Users/accountfiles/";
...
$mysqli->query("UPDATE users SET Profile_Pic = '".$newfilename."',email ='".$email."',Phone= '".$phone."', username = '".$_POST['username']."'  WHERE username = '".$_SESSION['username']."' ");
$mysqli->query("UPDATE OrderGroup SET Username = '".$_POST['username']."'  WHERE Username = '".$_SESSION['username']."' ");
$mysqli->query("UPDATE Orders SET Username = '".$_POST['username']."'  WHERE Username = '".$_SESSION['username']."' ");
...
$_SESSION['accountf'] ="Acount Info Saved!";
...
$phone = "";
    ...
    $mysqli->query("UPDATE users SET email ='".$email."',Phone= '".$phone."', username = '".$_POST['username']."' WHERE username = '".$_SESSION['username']."' ");
    $mysqli->query("UPDATE OrderGroup SET Username = '".$_POST['username']."'  WHERE Username = '".$_SESSION['username']."' ");
    $mysqli->query("UPDATE Orders SET Username = '".$_POST['username']."'  WHERE Username = '".$_SESSION['username']."' ");
    $_SESSION['username'] = $_POST['username'];
}
